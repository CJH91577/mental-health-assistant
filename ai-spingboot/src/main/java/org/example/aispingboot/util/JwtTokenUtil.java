...
public class JwtTokenUtil implements ApplicationContextAware {
    ...
    public static String extractTokenFromRequest(HttpServletRequest request) {
        ...
        // 优先使用配置的头部名称（默认 token）
        String headerName = "token";
        try {
            JwtConfig jwtConfig = getJwtConfig();
            if (StringUtils.hasText(jwtConfig.getHeader())) {
                headerName = jwtConfig.getHeader().trim();
            }
        ...
        if (StringUtils.hasText(tokenHeader)) {
            String prefix = "";
            try {
                prefix = getJwtConfig().getTokenPrefix();
            } catch (Exception ignored) {
            }
            if (StringUtils.hasText(prefix) && tokenHeader.startsWith(prefix)) {
                tokenHeader = tokenHeader.substring(prefix.length()).trim();
            }
            return tokenHeader;
        }
        return null;
    ...
    public static Integer getCurrentUserType() {
        ...
        if (roleType == null) {
            roleType = Integer.valueOf(jwt.getClaim("roleType").asString());
        }
        return roleType;
    }
    ...
    public static boolean isCurrentUserAdmin() {
        return Integer.valueOf(2).equals(getCurrentUserType());
    }
